...
>(
	(
		{ title, subtitle, selected = false, subtitleStyle, ...touchableProps },
		ref,
	) => {
		...
		return (
			...
			>
				{title && (
					<Text style={[theme.fonts.heading.xxs, styles.text]}>{title}</Text>
				)}
				{subtitle && (
					<Text style={[theme.fonts.body.m, styles.text, subtitleStyle]}>
						{subtitle}
					</Text>
				)}
			</TouchableNativeFeedback>
