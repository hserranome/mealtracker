...
import { Button, ButtonType, InputContainer, Separator, TextInput } from "../";
				...
				<View style={styles.container}>
					...
					<View>
						<TextInput
							...
							{...commonProps}
							onSubmitEditing={() =>
								form.setFocus("base_nutriments.energy_kcal")
							}
						/>
						<Button
							title="Scan barcode"
					...
					{nutritionFields.map((field, index) => (
						<TextInput
							...
							onSubmitEditing={() => {
								...
								if (nextField)
									form.setFocus(`base_nutriments.${nextField.name}`);
							}}
							returnKeyType={
								index === nutritionFields.length - 1 ? "done" : "next"
							}
