	...
	try {
		...
		return {
			...
			name:
				...
				item.product_name ||
				item.ecoscore_data.agribalyse.name_en,
			brands: item.brands,
			code: item.code,
...
export const searchProductBySearchTerm = async (
	searchTerm: string,
): Promise<Food[]> => {
	const response = await fetch(
	...
	return data.products.map(
		// biome-ignore lint/suspicious/noExplicitAny: This is a JSON response from an external API
		(product: any): Food => ({
			id: product.id,
			...
			base_serving_size: Number.parseFloat(product.product_quantity) || 100,
			base_serving_unit: "g",
			base_nutriments: {
