...
export const divideAmountWithPartsVariation = (
  ...
  minAmount: number,
  totalParts: number = 7,
  largerPartsCount: number = 2
) => {
  // Validate inputs
  if (total <= 0 || percentageIncrease <= 0 || minAmount <= 0) {
    throw new Error('Total, percentage increase, and minimum amount must be positive numbers.');
  }
  ...
  const baseAmount = total / (smallerPartsCount + largerPartsCount * increaseFactor);
