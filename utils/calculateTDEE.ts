...
export const calculateBaseTDEE = ({
	...
	const tdee =
		calculateBasalMetabolicRate(calculateBasalMetabolicRateArgs) *
		scale[activityLevel];
...
export const getWeeksToGoal = (
	...
	return Math.abs(
		round05((goalWeight - currentWeight) / weightVariancePerWeek),
	);
};
...
export const getDatesTdee = (
	...
	const averageTdee = Math.round(
		days.reduce((a, b) => a + b.tdee, 0) / days.length,
	);

	const averageWeight = roundFirstDecimal(
		days.reduce((a, b) => a + b.weight, 0) / days.length,
	);
